...
boolean CBoxAlgorithmP300StimulateurPattern::process(void)
{
	...
	//this->getLogManager() << LogLevel_Warning << "CBoxAlgorithmP300StimulateurPattern"<<"\n";
	...
	if(m_bStartReceived && !m_bHeaderSent)
	{
		...
		this->getLogManager() << LogLevel_Warning << "Stimulations generated: " <<l_stimSet.getStimulationCount();
		...
		//copy en fichier :  
		FILE * pFile;
		char* name = "d:\\config_P300FlashSequence.txt";
		//char* name = "config_P300FlashSequence.txt";
		this->getLogManager() << LogLevel_Warning << name;

		pFile = fopen (name, "w");
		if (pFile == NULL) this->getLogManager() << LogLevel_Error << "Could not open: " << name;
		else
		  {
